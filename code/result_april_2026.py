...
def generate_result():
    ...
    try:
        ...
        for location in range(0,no_of_files):
            ...
            print(f"Subjects Dimensions (Row and columns) {wb_subject[n].nrows} , {wb_subject[n].ncols }")
            """ 
            subjects_grades is a list of list, where each inner list contains the grades of all students for a particular subject. 
                So, subjects_grades[n] gives the list of grades of all students for the "nth" subject, and 
...
def check_current_sem(current_sem):
    ...
    sem = 0
    if "first" in sem_str or "one" in sem_str:
        sem=1
    elif "second" in sem_str or "two" in sem_str:
        sem=2
    elif "third" in sem_str or "three" in sem_str:
        sem=3
    elif "fourth" in sem_str or "four" in sem_str:
        sem=4
    elif "fifth" in sem_str or "five" in sem_str:
        sem=5
    elif "sixth" in sem_str or "six" in sem_str:
        sem=6
...
def createpdfs():
    ...
    for student in students_name:
        ...
        result_canvas.insert(result_index,Canvas(output_folder+"\\"+"_".join(str(student).title().strip().split(" "))+atkt_string+".pdf",pagesize=A4))
        ...
        result_canvas[result_index].drawCentredString(295,715,dept_name)
        #result_canvas[result_index].setFont("Helvetica-Bold",12)
        ...
        # else:
        result_canvas[result_index].setFont("Helvetica-Bold",14)
        result_canvas[result_index].drawCentredString(300,695,course_name)
        # if "master" not in course_name.lower():
        #     result_canvas[result_index].setFont("Helvetica-Bold",14)
        #     result_canvas[result_index].drawCentredString(300,685,degree_name)
        result_canvas[result_index].setFont("Helvetica-Bold",14)
        result_canvas[result_index].drawCentredString(300,675,"SEMESTER"+"-"+current_sem.upper()+","+" BATCH"+" "+batch_year)
        ...
        is_five = False
        if "five year" in course_branch.lower() or 'dual degree' in course_branch.lower():
            is_five = True
            if check_current_sem(current_sem) in [10]:
    ...
    for student in range(stud_count):
        ...
        for i in range(sub_count):
            ...
            if 'F' in subjects_grades[i][result_index]: # This condition is to check if student has failed in any subject, if yes then it will be considered as ATKT.
                ...
                # Currently this variable `fail_credits` (In March 2026) is not used, but can be used to show how many credits student has failed in.
            else:
                credits_secured_by_student+=course_credits[i]
            start_y = start_y-20
        if credits_secured_by_student < 12: 
            is_ATKT_fail = "FAIL"
        """ This is a condition to check if student has secured less than 12 credits in current semester and ATKT in any subject, then it will be considered as fail.
            This condition is based on the university rules (March 2026) and can be changed as per the requirement.
        """
        #else:
        #    is_ATKT_fail = "PASS"
...
note = tk.Label(
    root, text='Note : Please select the excel files of subject results and student_details and click on "Generate Results."', font=verdana_12)
#--------------------------------
today1 = date.today()
...
gen_result = tk.Button(root, text="Generate Results",font=("arial", 14),
                               command=generate_result, bg="green", fg="white", width=20)
...
ack_text = (
        ...
        "   DAVV, Indore\t\t DAVV, Indore\t\t DAVV, Indore\n"
    )
ack_lbl = tk.Label(root, text=ack_text, justify=LEFT, font=("Arial", 9))
...
ack_lbl.grid(row=18,column=0,columnspan=4)
